...
**发布日期:** 2022-09-24  
**原文链接:** https://www.cnblogs.com/morec/p/16724767.html  
**标签:** 无
...
![](./images/aspnetcore6.0源代码编译调试/image_1.png)

![](./images/aspnetcore6.0源代码编译调试/image_2.png)
...
![](./images/aspnetcore6.0源代码编译调试/image_3.png)

restore碰运气，如果红色错误多的话需要细看再细看，我遇到两个大的错误，一个是提示node的版本太老，就是我电脑版本是多少， 实际要求的版本是多少，我直接卸载重装了最新的node。该错误忘了截图

![](./images/aspnetcore6.0源代码编译调试/image_4.png)

看看这个报错，无非就是一堆vcxproj结尾的文件里导入 props的文件不存在。首选发现全是aspnetcore\src\Servers\IIS\AspNetCoreModuleV2这个文件夹下面的错误。

![](./images/aspnetcore6.0源代码编译调试/image_5.png)

打开其中一个文件看一下，按照VCTargetpath关键字每个文件都有一个，粗暴的注释掉。这个模块是iis的加载core模块，是c++写的,对运行起来调试代码基本无影响，我的项目现在都不用iis了。
...
![](./images/aspnetcore6.0源代码编译调试/image_6.png)

haixu ![](./images/aspnetcore6.0源代码编译调试/image_7.png)

restore需要几分钟时间的，最后就是这样的了（有错误不要紧，先自己看看能不能判断问题所在，不行就百度一下去查一查。）

![](./images/aspnetcore6.0源代码编译调试/image_8.png)

第六步 测试mvc源码 ,通过脚本打开mvc解决方案，startvs Mvc.sln，后续就是编译代码调试代码了。

 ![](./images/aspnetcore6.0源代码编译调试/image_9.png)

编译的时候发现这里有报错，懒得深究，调试有需要再解决了。再看注释很明显说不没有足够数据，crlf不能正确的到这里，就是不执行吧，那就粗暴的注释一下下了。

![](./images/aspnetcore6.0源代码编译调试/image_10.png)

结束！

hosting模块调试，一样执行cmd命令 startvs.cmd打开vs2022

![](./images/aspnetcore6.0源代码编译调试/image_11.png)

mvc模块调试

![](./images/aspnetcore6.0源代码编译调试/image_12.png)
