...
**发布日期:** 2024-12-04  
**原文链接:** https://www.cnblogs.com/morec/p/18586025  
**标签:** 无
...
![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_1.png)

这里登录的其实是https带证书的，但是kibana使用的是http的，所以在容器内部，config/elasticsearch.yml中需要把下面的两个参数置为false ，生产环境不建议这么操作。

![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_2.png)
...
![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_3.png)
...
![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_7.png)

- 3. 下面就是logstash安装跟mysql的同步了，测试数据如下：

![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_8.png)

 首先去logstash官网下载对应的包，我选的版本是8.16.1，目录如下是可以通过控制台执行的。

![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_9.png)

 这里只需要配置好mysql-connector的驱动和链接信息即可。

![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_10.png)
...
![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_12.png)

![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_13.png)
...
![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_14.png)

![](./images/elastic8.4.0搜索+logstash<->mysql实时同步+kibana可视化操作+netcore代码笔记/image_15.png)
