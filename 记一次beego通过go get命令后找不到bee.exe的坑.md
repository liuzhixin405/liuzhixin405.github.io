...
**发布日期:** 2022-06-24  
**原文链接:** https://www.cnblogs.com/morec/p/16410423.html  
**标签:** 无
...
![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_1.png)
```
第四步：go mod tidy整理代码

第五步：执行go install

到这里再回去gopath/bin查看，心心念念的bee.exe终于出来了，接下来该干嘛就干嘛了![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_2.png)

通过bee 新建mvc项目

![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_3.png)
...
![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_4.png)

设置以下三个变量，GOROOT即go安装目录，GOPATH即工作目录，GOBIN即执行go相关命令的路径，我的bee.exe就是拷贝到这个目录中去的：

![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_5.png)

![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_6.png)

最后一步设置：

![](./images/记一次beego通过go get命令后找不到bee.exe的坑/image_7.png)
